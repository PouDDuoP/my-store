...
jest.mock('../../../api/services/status.service', () => {
...
describe('Status Routes - Integration Tests (Admin Only)', () => {
  let app;
  ...
  beforeEach(() => {
    ...
    mockFindOne.mockResolvedValue({ id:1, name: 'Pending' });
    mockCreate.mockResolvedValue({ id:1, name: 'New Status' });
    mockUpdate.mockResolvedValue({ id:1, name: 'Updated Status' });
  ...
  describe('GET /api/v1/status', () => {
    test('should return 200 with status list (admin only)', async () => {
  ...
  describe('POST /api/v1/status', () => {
    test('should create status and return 201 (admin only)', async () => {
      const response = await request(app)
        .post('/api/v1/status')
        .set('Authorization', 'Bearer fake-token')
  ...
  describe('PATCH /api/v1/status/:id', () => {
    test('should update status and return 200 (admin only)', async () => {
      const response = await request(app)
  ...
  describe('DELETE /api/v1/status/:id', () => {
    test('should delete status and return 201 (admin only)', async () => {
      const response = await request(app)
  ...
  describe('GET /api/v1/status/:id with non-existent id', () => {
    test('should return 404 for non-existent status', async () => {
      mockFindOne.mockRejectedValue({ isBoom: true, output: { statusCode: 404, payload: { message: 'Status not found' } } });

      const response = await request(app)
        .get('/api/v1/status/99999')
        .set('Authorization', 'Bearer fake-token');
  ...
  describe('POST /api/v1/status with missing name', () => {
  ...
  describe('PATCH /api/v1/status/:id with tier profile', () => {
    test('should return 401 when tier tries to update status', async () => {
      // Set user to tier profile (not admin)
